...
Rotation. A camera sighting converted into field coordinates needs `cos` and `sin` the moment the robot is not facing straight down-field, and the honest place for that is after the trigonometry has been built. [Module 2](../02_trigonometry/) opens with the unit circle, derives rotation from it, and then does coordinate frames properly.
